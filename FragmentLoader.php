...
class FragmentLoader {
	static private function processString($s)
	{
		for($i = 1; $i <= 11; $i++) {
			$needle = 'val_'.$i.'="([^"]*)"\s+';
			if (preg_match('/val_'.$i.'="([^"]*)"/', $s, $match)) {
				$a[$i] = trim(html_entity_decode($match[1], ENT_QUOTES, 'UTF-8'));
			} else if ($i == 8) {
				// Hack fuer neues Fragmentlayout auf VroniPlag
				// val_8 (inLit) wurde entfernt
				$a[$i] = 'undefined';
			} else {
				return false;
			}
